...
import { ChevronDown, ChevronUp, SlidersHorizontal, ArrowDownUp, X, Check } from 'lucide-react';
...
function ShopContent() {
  ...
  const categorySlug = Array.isArray(routeParams.category)
    ? routeParams.category[0]
    : routeParams.category;

  // ── FIX: Read the ?search= query param that Navbar routes to this page ───
  const searchQuery = searchParams.get('search') || '';

  // Page title — show search term if present, otherwise category or default
        ...
        <aside className="hidden md:flex w-[240px] flex-shrink-0 flex-col sticky top-32 h-fit max-h-[calc(100vh-140px)] overflow-y-auto scrollbar-thin pr-2">
          ...
          <div className="pb-6 mb-6">
            <button onClick={() => setIsSizeOpen(!isSizeOpen)} className="w-full flex justify-between items-center mb-6 cursor-pointer">
              ...
              {isSizeOpen ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
            </button>
            {isSizeOpen && (
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-0">
              <div className="mb-3 md:mb-0">
                <div className="font-sans text-[10px] text-gray-400 tracking-widest uppercase mb-2 hidden md:block">
                  ...
                  <span>{searchQuery ? 'Search' : categorySlug ? 'Categories' : 'Shop'}</span>
                </div>
                <h1 className="font-serif text-3xl md:text-4xl text-primary-dark tracking-wide">{pageTitle}</h1>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 scrollbar-none">
              <div className="mb-8">
                ...
                <div className="flex flex-col space-y-3">
                  {fabricOptions.map(fabric => (
                    <div key={fabric} onClick={() => toggleFabric(fabric)} className="flex items-center space-x-3 cursor-pointer">
