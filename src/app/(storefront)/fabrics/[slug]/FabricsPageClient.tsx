...
function FabricsPageContent({ initialProducts, initialTotal, fabricName }: {
  ...
  const activeFilters = selectedSizes.length;
    ...
    <main className="min-h-screen bg-white pt-32 md:pt-40 pb-28 md:pb-20 relative">
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row gap-10">
        ...
        <section className="flex-1">
          {/* STICKY HEADER */}
          <div className="sticky top-32 md:top-40 z-30 bg-white pb-4 border-b border-gray-200">
            {/* Top row: count + sort */}
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-3">
              <div>
                ...
                <h1 className="font-serif text-3xl md:text-4xl text-primary-dark tracking-wide capitalize">{fabricName}</h1>
          ...
          {isLoading ? (
            ...
            <div className="py-20 text-center flex flex-col items-center justify-center border border-gray-100 bg-[#F9F9F9]">
              <span className="font-sans text-xs tracking-widest uppercase text-gray-500 mb-4">
                {searchQuery ? `No results found for "${searchQuery}".` : 'No products found in this fabric.'}
              </span>
              <button
                onClick={() => { setSelectedSizes([]); }}
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-x-4 md:gap-x-6 gap-y-10 md:gap-y-12 relative z-10 mt-4">
      ...
      {/* MOBILE FILTER SHEET — Size only */}
      {isMobileFilterOpen && (
        <div className="md:hidden fixed inset-0 z-[100] flex flex-col justify-end">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setIsMobileFilterOpen(false)} />
          <div className="relative bg-white w-full h-[70vh] rounded-t-2xl flex flex-col pt-4 animate-in slide-in-from-bottom-full duration-300">
...
export default function FabricsPageClient(props: {
  initialProducts: Product[];
  initialTotal: number;
  fabricName: string;
}) {
